...
VARIABLES 
\* @typeAlias: STATE = [ active: Int -> Bool, color: Int -> Str, counter: Int -> Int, pending:  Int -> Int, token: [pos: Int, q: Int, color: Str], InLoop: Bool];
\* @type: Int -> Bool;
...
AuxNext ==
  InLoop' \in {TRUE, FALSE} /\ (InLoop' = FALSE => InLoop = FALSE) (* fast *)
  \* \/  /\ ~InLoop  (* slow *)
  \*     /\ InLoop' \in {TRUE, FALSE}
  \* \/  InLoop' = InLoop

Next == (System \/ Environment) /\ AuxNext
...
Property(hist) ==
  LET \* @type: (Int, Int) => Int; 
    getSmallestLoopIndex(acc, nextIndex) ==
    IF acc < nextIndex /\ hist[acc].InLoop THEN acc ELSE nextIndex
  IN
  LET loopIndex == ApaFoldSet(getSmallestLoopIndex, 0, DOMAIN hist)
  IN
  \* LoopOK(hist, loopIndex) /\ LoopFair(hist, loopIndex) => Live(hist, loopIndex)
  \* (LoopOK(hist, loopIndex) /\ LoopFair(hist, loopIndex)) => ~SystemEnabled(hist[Len(hist)])
  (LoopOK(hist, loopIndex) /\ LoopFair(hist, loopIndex)) => Live(hist, loopIndex)
